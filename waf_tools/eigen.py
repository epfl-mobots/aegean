...
def options(opt):
    ...
    opt.add_option('--lapacke_blas', action='store_true',
                   help='enable lapacke/blas if found (required Eigen>=3.3)', dest='lapacke_blas')
...
def check_eigen(conf, *k, **kw):
    ...
    includes_check = ['/usr/include/eigen3',
        ...
        if conf.options.lapacke_blas:
            ...
            if world_version == 3 and major_version >= 3:
                ...
                if lapacke or blas_lib != '':
                    ...
                    conf.env.LIB_EIGEN = []
                    conf.end_msg('LAPACKE: \'%s\', BLAS: \'%s\'' %
                                 (lapacke_path, blas_path))
                elif lapacke:
                    conf.end_msg('Found only LAPACKE: %s' %
                                 lapacke_path, 'YELLOW')
                elif blas_lib != '':
                ...
                conf.end_msg('Found Eigen version %s: LAPACKE/BLAS can be used only with Eigen>=3.3' %
                             (str(world_version) + '.' + str(major_version) + '.' + str(minor_version)), 'RED')
    except:
        if required:
