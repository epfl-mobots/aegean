...
int main(int argc, char** argv)
{
    ...
    for (uint i = 0; i < dataframes.size(); ++i) {
        archive.save(dataframes[i].positions(),
            "seg_" + std::to_string(i) + "_reconstructed_positions");

        archive.save(dataframes[i].velocities(),
            "seg_" + std::to_string(i) + "_reconstructed_velocities");

        archive.save(dataframes[i].accelerations(),
            "seg_" + std::to_string(i) + "_reconstructed_accelerations");

        Eigen::MatrixXd features = dataframes[i].get_feature_matrix();
        for (uint j = 0; j < features.cols(); ++j) {
            archive.save(features.col(j),
                "seg_" + std::to_string(i) + "_" + feature_names[j]);
        }
    }

    {
        std::ofstream ofs(archive.dir_name() + "/num_centroids.dat");
        ofs << centroids << std::endl;
    }
    {
        std::ofstream ofs(archive.dir_name() + "/window_in_seconds.dat");
        ofs << window_in_seconds << std::endl;
    }
    {
        std::ofstream ofs(archive.dir_name() + "/fps.dat");
        ofs << fps << std::endl;
    }
